    ...
    pub fn from_raw_state_changes(
        ...
        let changes = changes
            ...
            .map(|RawStateChangesWithTrieKey { trie_key, changes }| {
                let value = changes
                    .last()
                    .expect("state_changes must not be empty")
                    .data
                    .clone();
...
/// Represents token price (numer=token, denom=USDT)
#[derive(
...
impl Role {
    /// returns `r`-prefixed key
    pub fn to_trie_key_component(&self) -> String {
        match self {
            Self::Admin => "ra".into(),
            Self::SetChargeFree => "rc".into(),
            Self::SetFee => "rf".into(),
            Self::SetPrice(symbol) => format!("rp:{}", symbol),
            Self::Mint(symbol) => format!("rm:{}", symbol),
            Self::Burn(symbol) => format!("rb:{}", symbol),
            Self::ChargeFree => "rd".into(),
        }
    }
}
...
impl std::fmt::Display for Role {
    fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
        match &self {
            ...
            Self::SetChargeFree => write!(f, "SetChargeFree"),
            Self::ChargeFree => write!(f, "ChargeFree"),
            Self::SetFee => write!(f, "SetFee"),
            Self::SetPrice(symbol) => write!(f, "SetPrice({})", symbol),
            Self::Mint(symbol) => write!(f, "Mint({})", symbol),
            Self::Burn(symbol) => write!(f, "Burn({})", symbol),
        }
    }
}
