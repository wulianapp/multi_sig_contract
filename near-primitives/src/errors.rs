...
pub enum ActionsValidationError {
    ...
    /// The specified fee symbol of deposit is unavailable
    DepositFeeSymbolUnavailable,
    /// The transaction includes actions with deposit of varied symbols.
    VariedtDepositSymbols,
    /// The transaction includes actions with deposit fee of varied symbols.
    VariedtDepositFeeSymbols,
}
...
impl Display for ActionsValidationError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> Result<(), std::fmt::Error> {
        match self {
            ...
            ActionsValidationError::DepositFeeSymbolUnavailable => write!(
                f,
                "The specified fee symbol of deposit is unavailable",
            ),
            ActionsValidationError::VariedtDepositSymbols => write!(
                f,
                "The actions deposit varied symbols",
            ),
            ActionsValidationError::VariedtDepositFeeSymbols => write!(
                f,
                "The actions deposit fee varied symbols",
            ),
        }
    }
}
...
pub enum ActionErrorKind {
    ...
    /// Some type of dynamic configuration actions required permission.
    DynamicConfigNoPermission { actor_id: AccountId },
    ...
    /// Delete account which is a treasury.
    DeleteAccountIsTreasury { account_id: AccountId },
    ...
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> Result<(), std::fmt::Error> {
        match self {
            ...
            InvalidTxError::NotEnoughBalance {
                ...
                signer_id,
                if let Some(symbol) = symbol { symbol } else { "NEAR" },
                balance,
                cost
            ),
...
impl Display for ActionErrorKind {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> Result<(), std::fmt::Error> {
        match self {
            ...
            ActionErrorKind::DynamicConfigNoPermission { actor_id } => write!(
                f,
                "Actor {:?} doesn't have permission to complete the action",
                actor_id
            ),
            ...
            ActionErrorKind::DeleteAccountIsTreasury { account_id } => {
                write!(f, "Account {:?} is a treasury and can not be deleted", account_id)
            }
