...
use crate::action::{Deposit, MintBurnAction, SetFeeAction, SetPriceAction, SetRoleAction};
...
use crate::types::{
    AccountId, AccountWithPublicKey, Balance, BalanceRatio, BlockHeight, EpochHeight, EpochId, Fee,
    FunctionArgs, Gas, Nonce, NumBlocks, Role, ShardId, StateChangeCause, StateChangeKind,
...
#[derive(serde::Serialize, serde::Deserialize, Debug, PartialEq, Eq, Clone)]
pub struct TokenBalanceInfoView {
    pub symbol: String,
    #[serde(with = "dec_format")]
    pub balance: Balance,
}
...
pub struct TokenBalanceList {
    pub tokens: Vec<TokenBalanceInfoView>,
}

impl FromIterator<TokenBalanceInfoView> for TokenBalanceList {
    fn from_iter<I: IntoIterator<Item = TokenBalanceInfoView>>(iter: I) -> Self {
        Self {
...
impl From<Action> for ActionView {
    fn from(action: Action) -> Self {
        match action {
            ...
            Action::SetRole(action) => ActionView::SetRole {
                role: action.role,
                enable: action.enable,
            },
            Action::SetFee(action) => ActionView::SetFee {
                symbol: action.symbol,
                fee: action.fee,
            },
            Action::SetPrice(action) => ActionView::SetPrice {
                symbol: action.symbol,
                price: action.price,
            },
            Action::MintBurn(action) => ActionView::MintBurn {
                symbol: action.symbol,
...
impl TryFrom<ActionView> for Action {
    ...
    fn try_from(action_view: ActionView) -> Result<Self, Self::Error> {
        Ok(match action_view {
            ...
            ActionView::SetRole { role, enable } => {
                Action::SetRole(Box::new(SetRoleAction { role, enable }))
            }
            ActionView::SetFee { symbol, fee } => {
                Action::SetFee(Box::new(SetFeeAction { symbol, fee }))
            }
            ActionView::SetPrice { symbol, price } => {
                Action::SetPrice(Box::new(SetPriceAction { symbol, price }))
            }
            ActionView::MintBurn {
                symbol,
                amount,
                mint,
            } => Action::MintBurn(Box::new(MintBurnAction {
                symbol,
                amount,
                mint,
            })),
